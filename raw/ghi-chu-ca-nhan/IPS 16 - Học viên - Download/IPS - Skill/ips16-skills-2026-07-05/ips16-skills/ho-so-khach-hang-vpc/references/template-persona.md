...
**Quy ước cờ kiểm chứng (chống bịa)** — mỗi item Job/Pain/Gain gắn 1 nhãn xuất xứ ở cuối dòng:
- `ref: [r###] {count}` = có quote khách thật (Mode B). `count` phải bằng số ref liệt kê.
- `[Mode A]` = suy từ kiến thức khóa, **chưa** có giọng khách thật.
- `⚠️ [GIẢ ĐỊNH]` = suy diễn, chưa có bằng chứng — **bắt buộc in cờ ⚠️** để anh Long rà lại, và đếm vào `n_assumptions` ở frontmatter.

Item không gắn được 1 trong 3 nhãn → **xóa khỏi hồ sơ**, không xuất.
...
n_assumptions: 0  # số item gắn ⚠️ [GIẢ ĐỊNH] (chưa kiểm chứng) — đọc kèm khi tin dùng hồ sơ
...
> ⚠️ **Mức độ kiểm chứng**: {{X}} item có quote thật · {{Y}} item `[Mode A]` · {{Z}} item `⚠️ [GIẢ ĐỊNH]`. Status: `{{status}}` · N = {{sample_size}}. {{Nếu Z>0 hoặc status≠validated → ghi: "Còn item suy đoán/chưa kiểm chứng — xác minh bằng giọng khách thật trước khi dùng cho sale page / quảng cáo."}}
