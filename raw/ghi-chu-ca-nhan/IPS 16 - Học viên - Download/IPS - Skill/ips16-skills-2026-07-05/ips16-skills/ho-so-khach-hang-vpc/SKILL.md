...
## 🚫 Chống bịa & Chống ảo giác — CỔNG BẮT BUỘC trước khi xuất file

Đây là cơ chế chống bịa **gom một chỗ**. Mọi quy tắc rải rác trong skill đều quy về đây. **Không file `persona-*.md` / `src-extraction-*.md` nào được xuất nếu chưa qua cổng này.**

### Nguyên tắc nền (áp dụng cả 2 mode)

1. **Chỉ ghi cái có thật trong nguồn.** Mode B: chỉ cái có trong raw. Mode A: chỉ cái suy ra được từ kiến thức khóa (Brain vault) hoặc anh Long xác nhận miệng. Không có nguồn → không ghi.
2. **Mọi item phải gắn xuất xứ.** Mode B → `ref: [r###]` truy về source file. Mode A → `[Mode A]`. Suy diễn → `⚠️ [GIẢ ĐỊNH]`. Item không gắn được 1 trong 3 nhãn này → **xóa, không xuất**.
3. **Không chế dữ kiện định lượng.** Cấm bịa `count`, `sample_size`, `%`, TAM, CVR. `count` của một item **phải bằng đúng số ref ID thực** liệt kê cho item đó — không phóng đại, không làm tròn lên. Số ước lượng (TAM…) ghi rõ "(ước lượng)".
4. **Không chế ref ID.** Mọi `[r###]` trong persona phải tồn tại thật trong source file. Cấm tạo ref mới khi tổng hợp hay khi xuất persona.
5. **Không chế quote.** Quote section 8 phải copy nguyên văn từ raw — không nhớ lại, không viết lại, không sửa chính tả, không "đẹp hóa". Không có raw → để trống + đánh dấu `chưa có`.
6. **Phân biệt "khách nói" vs "tôi suy ra".** Câu khách viết = dữ liệu. Diễn giải của tôi = suy diễn, bắt buộc nhãn `⚠️ [GIẢ ĐỊNH]` / `[suy diễn]`. Cấm viết "khách thường nói…" khi không có raw — đó là bịa giọng khách.

### Mode A nguy hiểm hơn Mode B — quy tắc riêng

Mode A sinh nội dung **từ trí nhớ** → đây là chỗ ảo giác cao nhất. Vì vậy:

- **Mọi item Mode A mặc định gắn `[Mode A]`** = chưa kiểm chứng bằng giọng khách thật.
- **Status trần của hồ sơ thuần Mode A = `hypothesis`** — không bao giờ `validated`.
- Khi đề xuất một Job/Pain/Gain mà **không** lấy được từ Brain vault → gắn thêm `⚠️ [GIẢ ĐỊNH]` và nói rõ với anh Long "đây là tôi suy đoán, không phải từ khóa".

### Checklist self-verify — chạy trước MỌI lần xuất file

Tôi phải tự rà và **in checklist này ra cho anh Long trước khi ghi file**. Mỗi dòng phải `✅`; nếu có `❌` → quay lại sửa, KHÔNG xuất:

- [ ] **Ref tồn tại** — mọi `[r###]` trong persona đều có trong source file (Mode B).
- [ ] **Count khớp** — mọi `count` = đúng số ref liệt kê, không phóng đại.
- [ ] **Quote nguyên văn** — từng quote section 8 đối chiếu đúng ký tự với raw gốc.
- [ ] **Nhãn xuất xứ đủ** — mọi item có 1 nhãn `[r###]` / `[Mode A]` / `⚠️ [GIẢ ĐỊNH]`.
- [ ] **Không số bịa** — mọi con số (sample_size, TAM, %, count) truy được về nguồn; số ước lượng ghi rõ "(ước lượng)".
- [ ] **Status đúng bằng chứng** — `validated` chỉ khi N≥30 + buyer≥10 + có quote; còn lại `hypothesis` / `draft`.
- [ ] **Cờ nghi ngờ hiển thị** — mọi item `confidence: low` / `[GIẢ ĐỊNH]` được in cờ `⚠️` trong file cuối + đếm vào `n_assumptions` của frontmatter.

Ghi kết quả checklist vào cuối `wiki/log.md` mỗi phiên (mục "Self-verify").
...
> **Trước tiên: chạy Cổng chống bịa** (mục 🚫 ở đầu skill) — in checklist self-verify, mọi dòng phải `✅`. Vì đây là hồ sơ thuần Mode A, mọi item phải gắn `[Mode A]`, item không từ Brain vault phải gắn `⚠️ [GIẢ ĐỊNH]`, và status trần là `hypothesis`.
...
> **Trước tiên: chạy Cổng chống bịa** (mục 🚫 ở đầu skill) — đối chiếu mọi `[r###]` có thật trong source, `count` = số ref thực, quote đúng nguyên văn raw. Mọi dòng checklist phải `✅` mới xuất.
...
- **Chế ref ID, chế số count, hoặc chế quote.** Mọi `[r###]` phải có thật trong source; `count` = số ref thực; quote copy nguyên văn từ raw. Không có raw → để trống, đánh dấu `chưa có`.
- **Xuất file khi chưa chạy Cổng chống bịa** (checklist self-verify ở đầu skill). Mọi dòng phải ✅.
- **Để item Mode A không gắn nhãn `[Mode A]` / `⚠️ [GIẢ ĐỊNH]`** — người đọc sau sẽ tưởng là dữ liệu khách thật.
